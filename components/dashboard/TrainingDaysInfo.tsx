...
export function TrainingDaysInfo({
	workoutDays,
	recentWorkouts,
	daysPerWeek,
}: TrainingDaysInfoProps) {
	...
	for (const workout of recentWorkouts) {
		...
		if (workoutDate >= eightWeeksAgo) {
			...
			if (
				!patterns[dayNumber].lastWorkout ||
	...
	for (let i = 0; i < 7; i++) {
		patterns[i].frequency =
			weekCount > 0 ? patterns[i].workoutCount / weekCount : 0;
	}
	...
	const allDays = Array.from({ length: 7 }, (_, i) => {
		...
		const workoutName = isCompletedThisWeek
			...
			: pattern.lastWorkout
				? pattern.lastWorkout.name ||
					pattern.lastWorkout.workoutDay?.name ||
					"Workout"
				: null;
	...
	const activeTrainingDays = allDays.filter(
		(day) => day.isActiveTrainingDay,
	).length;
			...
			<CardContent>
				<div className="space-y-4">
					<p className="text-sm text-muted-foreground">
						Based on your workout history, you typically train on{" "}
						{activeTrainingDays} days per week.
					</p>
							...
							>
								<div className="flex items-center">
									{day.isCompletedThisWeek ? (
										...
										<Dumbbell
											className={`h-4 w-4 mr-3 ${day.isToday ? "text-primary" : "text-muted-foreground"}`}
										/>
									) : (
									...
									<div>
										...
										{day.isCompletedThisWeek ? (
											...
											<div className="text-sm text-muted-foreground">
												{day.workoutName
													? `Usually: ${day.workoutName}`
													: "Training day"}
												{day.frequency > 0 &&
													` (${Math.round(day.frequency * 100)}%)`}
											</div>
										) : (
											<div className="text-sm text-muted-foreground">
												Rest day
										...
										{(day.isCompletedThisWeek || day.isActiveTrainingDay) &&
											day.workoutName &&
											patterns[day.dayNumber].lastWorkout?.workoutDay
												?.level && (
												<Link
