...
import { subDays, format, eachDayOfInterval, isSameDay, isAfter, isBefore } from "date-fns";
...
export function ConsistencyChart({ workouts, daysToShow = 28 }: ConsistencyChartProps) {
  ...
  const workoutCompletionRate = chartData.filter(d => d.isPast || d.isToday).length > 0
    ? Math.round((completedWorkouts.length / chartData.filter(d => d.isPast || d.isToday).length) * 100)
    : 0;
      ...
      <div className="bg-background text-foreground p-2 rounded-md border border-border shadow-md">
        ...
        {data.value ? (
          <>
            ...
            <p className="text-xs text-muted-foreground">{data.exerciseCount} exercises</p>
          </>
        ) : data.isPast ? (
        ...
        <div className="mt-4 pt-4 border-t">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <div className="font-medium">{workoutCompletionRate}% ({completedWorkouts.length} workouts)</div>
            </div>
          </div>
        </div>
      </CardContent>
