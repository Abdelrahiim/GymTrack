...
export function ExerciseProgressChart({
	...
	const exerciseColors = exerciseNames.reduce<Record<string, string>>(
		(colors, name, index) => {
	...
	return (
		<div className="h-full w-full flex flex-col">
			<Tabs
				...
				onValueChange={(value) =>
					setChartType(value as "weight" | "reps" | "volume")
				}
			>
									...
									<YAxis
										...
										label={{
											value:
												type === "reps"
													? chartTypeConfig[
															type as keyof typeof chartTypeConfig
														].label
													: (() => {
															...
															if (showMultipleExercises) {
																...
																for (const name of exerciseNames) {
																	const exerciseData =
																		exerciseProgressData[name] || [];
																	for (const point of exerciseData) {
																		if (point.weightUnit) {
																			allUnits[point.weightUnit] =
																				(allUnits[point.weightUnit] || 0) + 1;
																		}
																	}
																}
																// Find the most common unit
																const mostCommonUnit =
																	Object.entries(allUnits).sort(
																		(a, b) => b[1] - a[1],
																	)[0]?.[0] || "";
															...
															const exerciseName = exerciseNames[0];
															const exerciseData =
																exerciseProgressData[exerciseName] || [];
															const dataPointWithUnit = exerciseData.find(
																(point) => point.weightUnit,
															);
